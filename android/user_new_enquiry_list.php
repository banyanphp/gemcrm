...
if(isset($_REQUEST['user']))
{
	...
	if($user!=''){
			$get_team=mysqli_fetch_array(mysqli_query($db->connect(),"select team_id from tbl_team_members where members_code='$user'"));
			$team_id=$get_team['team_id'];
	}
	
	$authentication_check = "SELECT * FROM `tbl_enquiry` WHERE enq_team_id = '$team_id' AND `enq_status`='New'";
