...
$response = array();
if(isset($_REQUEST['user'])&& isset($_REQUEST['enq_no'])){

	$enq_user = $_REQUEST['user'];
	
	$id = $_REQUEST['enq_no'];
	$Date = date('Y-m-d H:i:s');

	if($enq_user!=''){
			$get_team=mysqli_fetch_array(mysqli_query($db->connect(),"select team_id from tbl_team_members where members_code='$enq_user'"));
			$team_id=$get_team['team_id'];
	}
	
		
	$query = "UPDATE `tbl_enquiry` SET `enq_alloted_to`='$enq_user',`enq_team_id`='$team_id',`enq_status`='Pending' WHERE `enq_no`='$id '";
			...
			$res=mysqli_query($db->connect(),$query);
				if($res){
					...
					$response["success"] ="1";
 $response["notification"] = "Alloted Successfully";
...
$q="select enq_company_name,enq_company_email,enq_no,enq_company_phn_no,enq_contact_person_name,enq_contact_person_phone_no from tbl_enquiry  WHERE `enq_id`='$id'";
					$qs=mysqli_fetch_array(mysqli_query($db->connect(),$q));
...
Our Product expert Mr/Mrs.'.$name3.' has been alloted to attend your enquiry.Our Expert contact details are Email-id: '.$email3.' and Mobile no is: '.$phone3.'.<br>
                                  Meanwhile if you have any questions .Please Contact us at <a href="mailto:sales@gemindia.com" style="font-weight:700;color:#46b08d;text-decoration:none" target="_blank">sales@gemindia.com</a> or call us on 9344455927
