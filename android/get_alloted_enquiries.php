...
if(isset($_REQUEST['user']))
{
	...
	$authentication_check = "SELECT * FROM `tbl_enquiry` WHERE `enq_alloted_to`='$user' AND `enq_status` = 'Pending' OR `enq_alloted_to`='$user' AND `enq_status` = 'Process'
ORDER BY `tbl_enquiry`.`enq_id`  DESC";
	...
	if (mysqli_num_rows($result1) > 0) {
		...
		while ($row1 = mysqli_fetch_assoc($result1)){
		 ...
		 $user["enq_product_id"] = $row1["enq_product_series"];
