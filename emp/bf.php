...
$enq	="SELECT * FROM `tbl_enquiry` WHERE `enq_no`='$enq_id'";
					...
					// date modifier for created on
						...
						$response['created_on'] = date( 'Y-m-d', $timestamp);
...
$user["process"].= $apointmnt_res['appointment_description'];
$user["created_on"]=$enq_process_update_on;
...
$user["created_on"]=$enq_process_update_on;
									...
									if($task_res['task_status']==1){
										$user['process'].= "Task is  Completed";
										$user['process'].= $task_res['task_report_description'];
										
									}
										else{
																					$user['process'].= "Task is Process";
